// -----JS CODE-----
// MyBirthdayController.js
// Version: 0.0.2
// Event: Initialized
// Description: The primary script that drives the my birthday template.

//@input bool extendedMode = false {"showIf": "extendedMode"}
...
// @ui {"widget":"group_end"}
// @ui {"widget":"group_start", "label":"Reveal Message", "showIf": "extendedMode"}
...
if (script.extendedMode) {
    colorsSet = [ script.primaryHatColor,
        script.secondaryHatColor,
        script.tertiaryHatColor ];
} else {
    colorsSet = [ script.primaryColor,
...
function setProps() {
    ...
    for (var j = 0; j < script.hatMaterials.length; j++) {
        script.hatMaterials[j].mainPass.baseColor = colorsSet[ j % colorsSet.length ];
    }
